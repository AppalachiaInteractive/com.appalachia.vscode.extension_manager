...
open with the `appa-extension-manager.registries` setting. This has the same format
...
You can manually select channels with the `appa-extension-manager.channels` settings
...
```JSON
"appa-extension-manager.channels": {
...
disable it with the `appa-extension-manager.updateCheckInterval` setting.
